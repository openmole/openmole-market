globals [
  ...
  burnt-final-trees ;; check if there have been percolation
  seed
]
...
to ignite  ;; patch procedure
  if random-float 1 > resistance [
  sprout-fires 1
    [ set color red ]
  set pcolor black
  set burned-trees burned-trees + 1
    if pxcor = max-pxcor [set burnt-final-trees burnt-final-trees + 1]
  ]
end
...
to-report binary-burnt
  ifelse burnt > 0.5 [ report 1][report 0]
end
...
RESISTANCE parameter : threshold for random draw if a tree can resist ignite
